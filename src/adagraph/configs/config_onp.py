...
from dataloaders.onp import ONP, ONPSampler
...
DOMAINS = [[x for x in range(7)]]
ACTUAL_TARGET_DOMAIN = 6
SOURCE_DOMAINS = [x for x in range(6)]
NUM_META = 1

DATALIST='../../data/ONP/processed/'
...
def init_loader(bs, domains=[], shuffle=False, auxiliar= False, size=224, std=[0.229, 0.224, 0.225]):
    ...
    dataset = ONP(DATALIST,domains=domains)
    ...
    if not auxiliar:
        ...
        return torch.utils.data.DataLoader(dataset, batch_size=bs, drop_last=False,num_workers=4, sampler=ONPSampler(dataset,bs))
