...
from dataloaders.elec import Elec, ElecSampler
...
DOMAINS = [[x for x in range(30)]]
ACTUAL_TARGET_DOMAIN = 29
SOURCE_DOMAINS = [x for x in range(29)]
NUM_META = 1
...
def init_loader(bs, domains=[], shuffle=False, auxiliar= False, size=224, std=[0.229, 0.224, 0.225]):
    ...
    dataset = Elec(DATALIST,domains=domains)
    ...
    if not auxiliar:
        ...
        return torch.utils.data.DataLoader(dataset, batch_size=bs, drop_last=False,num_workers=4, sampler=ElecSampler(dataset,bs))
